...
class TestSegmentProgress:
    def test_no_progress_file_returns_empty_dict(self, fileutils, tmp_path):
        work_path = tmp_path / "myvideo.mkv"
        work_path.write_bytes(b"dummy")

        assert fileutils.load_segment_progress(str(work_path)) == {}

    def test_save_then_load_roundtrip(self, fileutils, tmp_path):
        work_path = tmp_path / "myvideo.mkv"
        work_path.write_bytes(b"dummy")

        fileutils.save_segment_progress(str(work_path), {"seg1.mkv": "abc123"})
        assert fileutils.load_segment_progress(str(work_path)) == {"seg1.mkv": "abc123"}

    def test_progress_file_uses_dot_prefix_sidecar_name(self, fileutils, tmp_path):
        work_path = tmp_path / "myvideo.mkv"
        work_path.write_bytes(b"dummy")
    ...
    def test_clear_removes_progress_file(self, fileutils, tmp_path):
        work_path = tmp_path / "myvideo.mkv"
        work_path.write_bytes(b"dummy")

        fileutils.save_segment_progress(str(work_path), {"seg1.mkv": "abc123"})
        fileutils.clear_segment_progress(str(work_path))

        assert fileutils.load_segment_progress(str(work_path)) == {}

    def test_clear_on_nonexistent_file_does_not_raise(self, fileutils, tmp_path):
        work_path = tmp_path / "myvideo.mkv"
        work_path.write_bytes(b"dummy")
    ...
    def test_corrupt_progress_file_falls_back_to_empty_dict(self, fileutils, tmp_path):
        work_path = tmp_path / "myvideo.mkv"
        work_path.write_bytes(b"dummy")
    ...
    def test_progress_survives_multiple_segments(self, fileutils, tmp_path):
        """
